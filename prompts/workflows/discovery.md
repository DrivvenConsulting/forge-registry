Run Discovery (Phase 1): raw idea → spec file with functional requirements, technical constraints, edge cases, acceptance criteria, and success metrics.

Run the workflow with id **discovery**.

Follow the execution policy:

1. **Plan mode first:** Read the workflow definition from `.cursor/workflows/discovery/WORKFLOW.md`. Present the plan (steps and inputs from that file). Do not execute any step until I confirm the plan.
