Run Planning (Phase 2): spec from Phase 1 → GitHub issues with subtasks, each referencing at least one acceptance criterion.

Run the workflow with id **planning**.

Follow the execution policy:

1. **Plan mode first:** Read the workflow definition from `.cursor/workflows/planning/WORKFLOW.md`. Present the plan (steps and inputs from that file). Do not execute any step until I confirm the plan.
