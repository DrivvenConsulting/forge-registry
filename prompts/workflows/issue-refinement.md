Run issue refinement: turn a parent GitHub issue in Todo into [dev]/[ops]/[qa] sub-issues and move root to Ready.

Run the workflow with id **issue-refinement**.

Follow the execution policy:

1. **Plan mode first:** Read the workflow definition from `.cursor/workflows/issue-refinement/WORKFLOW.md`. Present the plan (steps and inputs from that file). Do not execute any step until I confirm the plan.
