Run Implementation (Phase 3): issue ID(s) → code commits and infrastructure changes.

Run the workflow with id **implementation**.

Follow the execution policy:

1. **Plan mode first:** Read the workflow definition from `.cursor/workflows/implementation/WORKFLOW.md`. Present the plan (steps and inputs from that file). Do not execute any step until I confirm the plan.
